...
/** Low-level pinned transport. Call through createSafeFetcher in production. */
...
function pageTitle(html: string) { return html.match(/<title[^>]*>([\s\S]*?)<\/title>/i)?.[1].replace(/\s+/g, " ").trim(); }
...
export async function collectScan(scan: ScanEvidence, fetcher: Fetcher = fetchPublic, activeLimits: ScanLimits = limits) {
  ...
  const homeEvidence: PageEvidence = { requestedUrl: scan.normalizedUrl, finalUrl: home.finalUrl, status: home.status, redirects: home.redirects, headers: home.headers, contentType: home.headers["content-type"], title: pageTitle(home.body), internalLinks: extractInternalLinks(home.body, homeUrl), durationMs: Date.now() - homeStarted }; scan.pages.push(homeEvidence);
  try { const robots = await fetcher(new URL("/robots.txt", homeUrl.origin), activeLimits.textBytes); scan.robots = { status: robots.status, discoveredSitemaps: [...robots.body.matchAll(/^sitemap:\s*(\S+)/gim)].map((match) => match[1]).slice(0, 10) }; } catch (error) { scan.robots = { discoveredSitemaps: [], error: error instanceof Error ? error.message : "robots fetch failed" }; }
  const declaredSitemap = scan.robots?.discoveredSitemaps.find((value) => { try { return new URL(value).origin === homeUrl.origin; } catch { return false; } });
  try { const sitemap = await fetcher(new URL(declaredSitemap ?? "/sitemap.xml", homeUrl.origin), activeLimits.sitemapBytes); scan.sitemap = { status: sitemap.status, selectedUrls: parseSitemapUrls(sitemap.body, homeUrl.origin, activeLimits.sitemapUrls) }; } catch (error) { scan.sitemap = { selectedUrls: [], error: error instanceof Error ? error.message : "sitemap fetch failed" }; }
  ...
  for (const candidate of candidates) { const started = Date.now(); try { const page = await fetcher(new URL(candidate), activeLimits.htmlBytes); scan.pages.push({ requestedUrl: candidate, finalUrl: page.finalUrl, status: page.status, redirects: page.redirects, headers: page.headers, contentType: page.headers["content-type"], title: pageTitle(page.body), internalLinks: extractInternalLinks(page.body, new URL(page.finalUrl)), durationMs: Date.now() - started }); } catch (error) { scan.pages.push({ requestedUrl: candidate, redirects: [], internalLinks: [], durationMs: Date.now() - started, error: error instanceof Error ? error.message : "Page fetch failed" }); } }
  return scan;
}
