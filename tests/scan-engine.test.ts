...
test("rejects malformed and non-HTTP URL schemes before scanning", async () => { for (const value of ["not a url", "file:///etc/passwd", "ftp://example.com"]) await assert.rejects(normalizePublicUrl(value), UnsafeUrlError); });
